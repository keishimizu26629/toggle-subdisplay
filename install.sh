...
echo ""
echo "Option 1a: Add to PATH temporarily (this session only)"
echo "  export PATH=\"$TEMP_DIR:\$PATH\""
echo "  toggle-subdisplay -q"
echo ""
echo "Option 1b: Add to PATH permanently (via shell config)"
echo "  echo 'export PATH=\"$TEMP_DIR:\$PATH\"' >> ~/.zshrc"
echo "  source ~/.zshrc"
echo "  toggle-subdisplay -q"
echo ""
echo "Option 2a: Install to system (permanent, requires sudo)"
echo "  sudo cp $TEMP_DIR/toggle-subdisplay /usr/local/bin/"
echo "  toggle-subdisplay -q"
echo ""
echo "Option 2b: Install to user bin (permanent, no sudo required)"
echo "  mkdir -p ~/bin"
echo "  cp $TEMP_DIR/toggle-subdisplay ~/bin/"
